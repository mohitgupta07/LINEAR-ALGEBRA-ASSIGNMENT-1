...
def comp(T):
    for _ in range(T):
        print("Case",_,checkTIME())
def checkTIME(c1='blue',c2='orange',ttl='Python Implementation v/s Numpy API',xl='Size of Matrix',yl='Time in Seconds'):
    X,Y1,Y2 = [],[],[]
    size,loop = 10,10
    ...
    Y1 = np.array(Y1)
    Y2 = np.array(Y2)
    #Y2 = Y2 * (Y1.mean()/Y2.mean())
    Y2 = np.exp( np.log(Y2) * (np.log(Y1).mean()/np.log(Y2).mean()) )
